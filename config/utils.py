...
def make_path2():
    return "{}_bs{}_fix={}_lr={}{}srcc".format(opts.logfile_name, opts.BatchSize, opts.fix_rate, opts.lr, opts.lr_decay_style)
def make_path3():
    return "{}_bs{}_fix={}_lr={}{}loss".format(opts.logfile_name, opts.BatchSize, opts.fix_rate, opts.lr, opts.lr_decay_style)
def make_path_llm():
    return "{}_bs{}_fix={}_lr={}{}llm".format(opts.logfile_name, opts.BatchSize, opts.fix_rate, opts.lr, opts.lr_decay_style)

""" ==================== Models ======================== """

def save_model(model):
    save_path = make_path2()
    ...
    model_name = os.path.join(config['checkpoint_base'], save_path, 'best_lr={}acc.pt'.format(opts.lr))
    torch.save(model.state_dict(), model_name)
def save_model2(model):
    save_path = make_path2()
    if not os.path.isdir(os.path.join(config['checkpoint_base'], save_path)):
        os.makedirs(os.path.join(config['checkpoint_base'], save_path), exist_ok=True)
    model_name = os.path.join(config['checkpoint_base'], save_path, 'best_lr={}loss.pt'.format(opts.lr))
def save_model_llm(model):
    save_path = make_path_llm()
    if not os.path.isdir(os.path.join(config['checkpoint_base'], save_path)):
        os.makedirs(os.path.join(config['checkpoint_base'], save_path), exist_ok=True)
    model_name = os.path.join(config['checkpoint_base'], save_path, 'best_lr={}.pt'.format(opts.lr))
    torch.save(model.state_dict(), model_name)
def save_model_srocc(model):
    save_path = make_path2()
    if not os.path.isdir(os.path.join(config['checkpoint_base'], save_path)):
        os.makedirs(os.path.join(config['checkpoint_base'], save_path), exist_ok=True)
    model_name = os.path.join(config['checkpoint_base'], save_path, 'best_lr={}.pt'.format(opts.lr))
    torch.save(model.state_dict(), model_name)
def save_model_loss(model):
    save_path = make_path3()
    if not os.path.isdir(os.path.join(config['checkpoint_base'], save_path)):
        os.makedirs(os.path.join(config['checkpoint_base'], save_path), exist_ok=True)
    model_name = os.path.join(config['checkpoint_base'], save_path, 'best_lr={}.pt'.format(opts.lr))
    torch.save(model.state_dict(), model_name)
def save_mlp(model):
    ...
    if not os.path.isdir(os.path.join(config['checkpoint_base'], save_path)):
        os.makedirs(os.path.join(config['checkpoint_base'], save_path), exist_ok=True)
    model_name = os.path.join(config['checkpoint_base'], save_path, 'best_lr={}mlp.pt'.format(opts.lr))
    torch.save(model.state_dict(), model_name)
